...
tasks.withType<Jar> {
    manifest {
        ...
        attributes["Build-Version"] = project.findProperty("gitVersion")?.toString()
        attributes["Bundle-Version"] = project.version
        attributes["Bundle-Name"] = project.findProperty("archivesBaseName")
...
tasks.register("publishArchives") {
    ...
    val gitVersion = (project.findProperty("gitVersion") as Provider<*>?)?.get().toString()
    val gitVersionMatch = gitVersion == version
    doFirst {
        if (!gitVersionMatch) {
            val cause =
                """
