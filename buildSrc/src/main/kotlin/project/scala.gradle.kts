...
tasks.register("scalaCheck") {
    ...
    dependsOn("clean", "compileTestScala", "integrationTestTask", "check")
...
tasks.findByName("docs")?.dependsOn(tasks.findByName("scaladoc"))

tasks.named<Delete>("clean") {
    delete.add(rootProject.file("build/docs/"))
}

afterEvaluate {

    configureMavenPublication {
        artifactId = "${base.archivesName.get()}_${scalaVersion}"
    }
    ...
    tasks.withType<ScalaCompile> {
        doFirst {
            println("Compiling using scala version: $scalaVersion")
        }
