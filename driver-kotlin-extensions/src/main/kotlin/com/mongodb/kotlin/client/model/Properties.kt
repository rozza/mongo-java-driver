...
internal val pathCache: MutableMap<Pair<Class<*>?, String>, String>
    by lazySoft { ConcurrentHashMap<Pair<Class<*>?, String>, String>() }
...
public fun <T> KProperty<T>.path(): String {
    val key = Pair(this.javaField!!.declaringClass, this.name)
    var path = pathCache.get(key)
    if (path == null) {
        val serialName = this.annotations.firstOrNull { it.annotationClass.simpleName == "SerialName" }
        path = serialName?.annotationClass?.members?.firstOrNull { it.name == "value" }?.call(serialName) as String?

        val constructorProperty = key.first.kotlin.primaryConstructor?.findParameterByName(this.name)
        path = path ?: constructorProperty?.annotations?.filterIsInstance<BsonId>()?.firstOrNull()?.let { "_id" }
