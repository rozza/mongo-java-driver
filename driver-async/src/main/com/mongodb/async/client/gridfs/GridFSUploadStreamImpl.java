...
final class GridFSUploadStreamImpl implements GridFSUploadStream {
    ...
    public void abort(final SingleResultCallback<Void> callback) {
        ...
        if (isClosedOrIsWriting(errorHandlingCallback)) {
            return;
        }
    ...
    public void write(final ByteBuffer src, final SingleResultCallback<Integer> callback) {
        notNull("src", src);
        notNull("callback", callback);
        write(src.remaining() == 0 ? -1 : src.remaining(), src, errorHandlingCallback(callback));
    }
    ...
    private void write(final int amount, final ByteBuffer src, final SingleResultCallback<Integer> callback) {
        if (isClosedOrIsWriting(callback)){
            return;
        }
    ...
    private <T> boolean isClosedOrIsWriting(final SingleResultCallback<T> errorHandlingCallback) {
        if (checkClosed()) {
    ...
    private boolean getWritingLock() {
    ...
    private <T> void callbackIsWritingException(final SingleResultCallback<T> callback) {
        callback.onResult(null, new MongoGridFSException("The AsyncOutputStream is in the process of writing"));
    }
}
