...
final class GridFSDownloadStreamImpl implements GridFSDownloadStream {
    ...
    private void getChunk(final int startChunkIndex, final SingleResultCallback<Document> callback) {
        if (resultsQueue.isEmpty()) {
            ...
            Document chunk = resultsQueue.poll();
            if (chunk.getInteger("n") != startChunkIndex) {
                chunkNotFound(startChunkIndex, callback);
            } else {
                callback.onResult(chunk, null);
            }
        }
    }

    private void chunkNotFound(final int startChunkIndex, final SingleResultCallback<Document> callback) {
    ...
    private void getBufferFromChunk(final Document chunk, final int expectedChunkIndex, final SingleResultCallback<byte[]> callback) {
        if (chunk == null || chunk.getInteger("n") != expectedChunkIndex) {
            callback.onResult(null, new MongoGridFSException(format("Could not find file chunk for file_id: %s at chunk index %s.",
                    fileInfo.getId(), expectedChunkIndex)));
            return;
        ...
        if (extraChunk && data.length > expectedDataLength) {
            callback.onResult(null, new MongoGridFSException(format("Extra chunk data for file_id: %s. Unexpected chunk at chunk index %s."
                    + "The size was %s and it should be %s bytes.", fileInfo.getId(), expectedChunkIndex, data.length,
                    expectedDataLength)));
    ...
    private <T> void callbackIsReadingException(final SingleResultCallback<T> callback) {
        callback.onResult(null, new MongoGridFSException("The AsyncInputStream is in the process of reading"));
    }
