...
class MongoDatabaseSpecification extends Specification {
    ...
    def 'should behave correctly when using withCodecRegistry'() {
        ...
        when:
        def collection = new MongoDatabaseImpl(name, codecRegistry, readPreference, writeConcern,
                executor).withCodecRegistry(newCodecRegistry)

        then:
        expect collection, isTheSameAs(new MongoDatabaseImpl(name, newCodecRegistry, readPreference, writeConcern, executor))
    }
    ...
    def 'should behave correctly when using withReadPreference'() {
        ...
        then:
        expect collection, isTheSameAs(new MongoDatabaseImpl(name, codecRegistry, newReadPreference, writeConcern, executor))
    }
    ...
    def 'should behave correctly when using withWriteConcern'() {
        ...
        then:
        expect collection, isTheSameAs(new MongoDatabaseImpl(name, codecRegistry, readPreference, newWriteConcern, executor))
    }
