...
class AggregateFluentSpecification extends Specification {
    ...
    def 'should build the expected AggregationOperation'() {
        ...
        def fluentAggregation = new AggregateFluentImpl<Document>(namespace, Document, codecRegistry, readPreference, executor,
                pipeline)

        when: 'default input should be as expected'
        fluentAggregation.into([]) { result, t -> }
        ...
        fluentAggregation.maxTime(999, MILLISECONDS).useCursor(true).into([]) { result, t -> }
    ...
    def 'should build the expected AggregateToCollectionOperation'() {
        ...
        new AggregateFluentImpl<Document>(namespace, Document, codecRegistry, readPreference, executor,
                pipeline)
                .batchSize(99)
                .maxTime(999, MILLISECONDS)
                .allowDiskUse(true)
    ...
    def 'should handle exceptions correctly'() {
        ...
        def fluentAggregation = new AggregateFluentImpl<BsonDocument>(namespace, BsonDocument, codecRegistry, readPreference, executor,
                pipeline)

        def futureResultCallback = new FutureResultCallback<List<BsonDocument>>()

        when: 'The operation fails with an exception'
        fluentAggregation.into([], futureResultCallback)
        futureResultCallback.get()
        ...
        fluentAggregation.toCollection(new FutureResultCallback())
