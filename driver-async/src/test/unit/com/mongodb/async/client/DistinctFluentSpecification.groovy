...
class DistinctFluentSpecification extends Specification {
    ...
    def 'should build the expected DistinctOperation'() {
        ...
        def fluentDistinct = new DistinctFluentImpl<Document>(namespace, Document, codecRegistry, readPreference, executor, 'field')

        when: 'default input should be as expected'
        fluentDistinct.into([]) { result, t -> }
        ...
        fluentDistinct.filter(new Document('field', 1)).maxTime(999, MILLISECONDS).into([]) { result, t -> }
    ...
    def 'should handle exceptions correctly'() {
        ...
        def fluentDistinct = new DistinctFluentImpl(namespace, BsonDocument, codecRegistry, readPreference, executor, 'field')

        def futureResultCallback = new FutureResultCallback()

        when: 'The operation fails with an exception'
        fluentDistinct.into([], futureResultCallback)
        ...
        fluentDistinct.filter(new Document('field', 1)).into([], futureResultCallback)
        futureResultCallback.get()
    ...
    def 'should follow the MongoIterable interface as expected'() {
        ...
        def mongoIterable = new DistinctFluentImpl<Document>(namespace, Document, codecRegistry, readPreference, executor, 'field')
