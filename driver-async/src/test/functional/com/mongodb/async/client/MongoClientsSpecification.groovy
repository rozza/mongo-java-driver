...
class MongoClientsSpecification extends FunctionalSpecification {
    ...
    def 'should respect the streamType over the system properties'() {
        ...
        when:
        def client = MongoClients.create(uri)
        def clazz = streamFactoryFactory

        then:
        client.settings.getStreamFactoryFactory().getClass() == streamFactoryFactory

        cleanup:
        client?.close()
        if (asyncType != null) System.setProperty('org.mongodb.async.type', asyncType)

        where:
        uri                                     |  systemType | streamFactoryFactory
        'mongodb://localhost/?streamType=nio2'  | 'netty'     | AsynchronousSocketChannelStreamFactoryFactory.class
        'mongodb://localhost/?streamType=netty' | 'nio2'      | NettyStreamFactoryFactory.class
    }
