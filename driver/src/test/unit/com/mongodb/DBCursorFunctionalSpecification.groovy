...
class DBCursorFunctionalSpecification extends FunctionalSpecification {
    ...
    def 'should use provided hints for count'() {
        ...
        when:
        collection.createIndex(new BasicDBObject('a', 1), new BasicDBObject('sparse', true));

        then:
        collection.find(new BasicDBObject('a', 1)).hint('a_1').count() == serverVersionAtLeast(asList(2, 6, 0)) ? 1 : 2
