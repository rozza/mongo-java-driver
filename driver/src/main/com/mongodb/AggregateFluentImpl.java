...
class AggregateFluentImpl<T> implements AggregateFluent<T> {
    ...
    private MongoIterable<T> execute() {
        ...
        if (outCollection != null) {
            ...
            return new FindOperationIterable<T>(new FindOperation<T>(new MongoNamespace(namespace.getDatabaseName(),
    ...
    private class FindOperationIterable<D> extends OperationIterable<D> {
        private final FindOperation<D> operation;
        private final ReadPreference readPreference;

        FindOperationIterable(final FindOperation<D> operation, final ReadPreference readPreference) {
        ...
        @Override
        public D first() {
            FindOperation<D> findFirstOperation = new FindOperation<D>(operation.getNamespace(), operation.getDecoder())
                                                      .filter(operation.getFilter())
                                                      .modifiers(operation.getModifiers())
                                                      .projection(operation.getProjection())
                                                      .maxTime(operation.getMaxTime(MILLISECONDS), MILLISECONDS)
                                                      .skip(operation.getSkip())
                                                      .sort(operation.getSort())
                                                      .tailableCursor(operation.isTailableCursor())
                                                      ...
                                                      .batchSize(0)
                                                      .limit(-1);
            return new OperationIterable<D>(findFirstOperation, readPreference).first();
        }
    }
}
