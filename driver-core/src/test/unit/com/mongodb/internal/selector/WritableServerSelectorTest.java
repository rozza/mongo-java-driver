...
class WritableServerSelectorTest {
    ...
    static Stream<Arguments> shouldChoosePrimaryServerInMultipleMode() {
        return Stream.of(
                Arguments.of(
    ...
    @ParameterizedTest
    @MethodSource
    void shouldChoosePrimaryServerInMultipleMode(final List<ServerDescription> expectedServerList,
    ...
    @ParameterizedTest
    @MethodSource
    void shouldChooseSecondaryServerInSingleMode(final List<ServerDescription> expectedServerList,
                                                 final ClusterDescription clusterDescription) {
        WritableServerSelector selector = new WritableServerSelector();
        assertEquals(expectedServerList, selector.select(clusterDescription));
    }

    static Stream<Arguments> shouldChooseSecondaryServerInSingleMode() {
        return Stream.of(
                Arguments.of(
                        Collections.singletonList(SECONDARY_SERVER),
                        new ClusterDescription(SINGLE, ClusterType.REPLICA_SET, Collections.singletonList(SECONDARY_SERVER)))
        );
    }
}
