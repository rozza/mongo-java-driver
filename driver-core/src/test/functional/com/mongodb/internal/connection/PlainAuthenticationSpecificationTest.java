...
class PlainAuthenticationSpecificationTest {

    @BeforeAll
    static void beforeAll() {
        assumeTrue(getCredential() != null && getCredential().getAuthenticationMechanism() == PLAIN);
    }
    ...
    void shouldThrowMongoSecurityExceptionWhenAuthenticationFailsSync() {
        InternalStreamConnection connection = createConnection(false,
                createPlainCredential("wrongUserName", "$external", "wrongPassword".toCharArray()));
        try {
    ...
    private static Authenticator createAuthenticator(final MongoCredential credential) {
        return credential == null ? null
                : new PlainAuthenticator(new MongoCredentialWithCache(credential), getClusterConnectionMode(), null);
    }
