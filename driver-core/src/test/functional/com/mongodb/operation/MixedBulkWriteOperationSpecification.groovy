...
class MixedBulkWriteOperationSpecification extends OperationFunctionalSpecification {
    ...
    def 'when a updating with an empty document, update should throw IllegalArgumentException'() {
        given:
        def id = new ObjectId()
        def op = new MixedBulkWriteOperation(getNamespace(),
    ...
    def 'when a updating with an invalid document, update should throw IllegalArgumentException'() {
        given:
        def id = new ObjectId()
        def op = new MixedBulkWriteOperation(getNamespace(),
