...
class SingleBatchCursor<T> implements BatchCursor<T> {
    ...
    static <R> SingleBatchCursor<R> createEmptyBatchCursor(final ServerAddress serverAddress, final int batchSize) {
        return new SingleBatchCursor<>(emptyList(), serverAddress, batchSize);
    }
    ...
    SingleBatchCursor(final List<T> batch,  final ServerAddress serverAddress, final int batchSize) {
        this.batch = batch;
