...
public final class PublisherCreator {
    ...
    public static <T> ListCollectionsPublisher<T> createListCollectionsPublisher(@Nullable final ClientSession clientSession,
        ...
        return new ListCollectionsPublisherImpl<>(clientSession, databaseName, resultClass, codecRegistry, readPreference, executor,
                                                  retryReads, collectionNameOnly);
    }
    ...
    public static <D, T> AggregatePublisher<T> createAggregatePublisher(@Nullable final ClientSession clientSession,
        ...
        return createAggregatePublisher(clientSession, new MongoNamespace(databaseName, "ignored"), documentClass, resultClass,
                                        codecRegistry, readPreference, readConcern, writeConcern, executor, pipeline, aggregationLevel,
                                        retryReads);
    }
    ...
    public static Mono<Void> createCreateCollectionMono(@Nullable final ClientSession clientSession,
                                                        final MongoNamespace namespace,
                                                        final CodecRegistry codecRegistry,
                                                        final ReadConcern readConcern,
                                                        final WriteConcern writeConcern,
        ...
        return createWriteOperationMono(() -> {
            CreateCollectionOperation operation =
                    new CreateCollectionOperation(namespace.getDatabaseName(), namespace.getCollectionName(), writeConcern)
    ...
    public static Mono<Void> createViewMono(@Nullable final ClientSession clientSession,
                                            final MongoNamespace namespace,
                                            final CodecRegistry codecRegistry,
                                            final ReadConcern readConcern,
                                            final WriteConcern writeConcern,
    ...
    public static Mono<Void> createDropDatabaseMono(@Nullable final ClientSession clientSession,
                                                    final String databaseName,
                                                    final ReadConcern readConcern,
                                                    final WriteConcern writeConcern,
                                                    final OperationExecutor executor) {
        return createWriteOperationMono(() -> new DropDatabaseOperation(databaseName, writeConcern), clientSession, readConcern, executor);
    }

    public static <T> Mono<T> createRunCommandMono(@Nullable final ClientSession clientSession,
                                                   final String databaseName,
                                                   final Class<T> resultClass,
                                                   final CodecRegistry codecRegistry,
                                                   final ReadPreference readPreference,
        ...
        return createReadOperationMono(
                ...
                clientSession, readPreference, readConcern, executor);
    }

    public static Flux<String> createIndexesFlux(
            final Supplier<AsyncWriteOperation<Void>> operationSupplier,
            @Nullable final ClientSession clientSession,
            final ReadConcern readConcern,
            final OperationExecutor executor,
            final List<IndexModel> indexes,
            final CodecRegistry codecRegistry) {
        return createWriteOperationMono(operationSupplier, clientSession, readConcern, executor)
                .thenMany(Flux.fromIterable(IndexHelper.getIndexNames(indexes, codecRegistry)));
    }

    public static Mono<BulkWriteResult> createSingleWriteRequestMono(
            final Supplier<AsyncWriteOperation<BulkWriteResult>> operation,
            @Nullable final ClientSession clientSession,
            final ReadConcern readConcern,
            final OperationExecutor executor,
            final WriteRequest.Type type) {
        return createWriteOperationMono(operation, clientSession, readConcern, executor)
    ...
    public static <T> Mono<T> createReadOperationMono(
            final Supplier<AsyncReadOperation<T>> operation,
            @Nullable final ClientSession clientSession,
            final ReadPreference readPreference,
            final ReadConcern readConcern,
            final OperationExecutor executor) {
        return Mono.from(new ReadOperationPublisher<>(operation.get(), clientSession, readPreference, readConcern, executor));
    }

    public static <T> Mono<T> createWriteOperationMono(
            final Supplier<AsyncWriteOperation<T>> operation,
            @Nullable final ClientSession clientSession,
            final ReadConcern readConcern,
            final OperationExecutor executor) {
        return Mono.from(new WriteOperationPublisher<>(operation.get(), clientSession, readConcern, executor));
    }
    ...
    @Nullable
    public static AsyncClientSession getSession(@Nullable final ClientSession clientSession) {
        return clientSession != null ? clientSession.getWrapped() : null;
    }
