...
public abstract class BatchCursorPublisher<T> implements Publisher<T> {
    ...
    abstract Function<AsyncOperations<?>, TimeoutContext> getTimeoutContext();
    ...
    Mono<BatchCursor<T>> batchCursor(final Supplier<AsyncReadOperation<AsyncBatchCursor<T>>> supplier) {
        return mongoOperationPublisher.createReadOperationMono(getTimeoutContext(), supplier, clientSession).map(BatchCursor::new);
    }
