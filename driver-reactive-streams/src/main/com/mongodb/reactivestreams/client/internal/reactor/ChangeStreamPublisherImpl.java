...
final class ChangeStreamPublisherImpl<T>  extends MongoBatchCursorPublisher<ChangeStreamDocument<T>> implements ChangeStreamPublisher<T> {
    ...
    public <TDocument> Publisher<TDocument> withDocumentClass(final Class<TDocument> clazz) {
        return new MongoBatchCursorPublisher<TDocument>(getClientSession(), getExecutor(), getReadConcern(), getReadPreference(),
                getRetryReads()) {
            @Override
