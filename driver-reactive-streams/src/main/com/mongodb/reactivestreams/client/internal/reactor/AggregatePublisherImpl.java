...
final class AggregatePublisherImpl<D, T> extends MongoBatchCursorPublisher<T> implements AggregatePublisher<T> {
    ...
    public Publisher<Void> toCollection() {
        ...
        return Mono.create(callback -> getExecutor().execute(operations.aggregateToCollection(pipeline, maxTimeMS, allowDiskUse,
                bypassDocumentValidation, collation, hint, comment, aggregationLevel), getReadConcern(),
