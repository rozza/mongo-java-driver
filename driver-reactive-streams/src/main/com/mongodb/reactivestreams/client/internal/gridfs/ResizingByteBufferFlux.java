...
class ResizingByteBufferFlux extends Flux<ByteBuffer> {
    ...
    ResizingByteBufferFlux(final Publisher<ByteBuffer> source, final int chunkSize) {
            ...
            BaseSubscriber<ByteBuffer> subscriber = new BaseSubscriber<ByteBuffer>() {
                ...
                protected void hookOnNext(@NonNull final ByteBuffer value) {
                    if (remainder == null || remainder.remaining() == 0) {
                        ...
                        copyByteBuffer(remainder.asReadOnlyBuffer(), newBuffer);
                        copyByteBuffer(value.asReadOnlyBuffer(), newBuffer);
                        ((Buffer) newBuffer).flip();
                        remainder = newBuffer;
                    }

                    while (remainder != null && remainder.remaining() >= chunkSize) {
