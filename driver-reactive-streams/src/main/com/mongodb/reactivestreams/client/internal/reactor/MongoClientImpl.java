...
import com.mongodb.reactivestreams.client.internal.Crypt;
import com.mongodb.reactivestreams.client.internal.Crypts;
...
public final class MongoClientImpl implements MongoClient {
    ...
    public MongoDatabase getDatabase(final String name) {
        return new MongoDatabaseImpl(name, getCodecRegistry(), getSettings().getReadPreference(),
                getSettings().getReadConcern(), getSettings().getWriteConcern(), getExecutor(), getSettings().getRetryReads(),
    ...
    public Publisher<String> listDatabaseNames() {
        return Flux.from(listDatabases()).filter(d -> d.containsKey("name")).map(d -> d.getString("name"));
    }

    @Override
    public Publisher<String> listDatabaseNames(final ClientSession clientSession) {
        return Flux.from(listDatabases(clientSession)).filter(d -> d.containsKey("name")).map(d -> d.getString("name"));
    }
    ...
    public <T> ListDatabasesPublisher<T> listDatabases(final ClientSession clientSession, final Class<T> clazz) {
        return PublisherCreator.createListDatabasesPublisher(clientSession, clazz, getCodecRegistry(),
                ReadPreference.primary(), getExecutor(), getSettings().getRetryReads());
    }
    ...
    public <T> ChangeStreamPublisher<T> watch(final List<? extends Bson> pipeline, final Class<T> resultClass) {
        return PublisherCreator.createChangeStreamPublisher(null, "admin", resultClass,
                getSettings().getCodecRegistry(), getSettings().getReadPreference(), getSettings().getReadConcern(), getExecutor(), pipeline,
    ...
    @Override
    public <T> ChangeStreamPublisher<T> watch(final ClientSession clientSession, final List<? extends Bson> pipeline, final Class<T> resultClass) {
    ...
    public Publisher<ClientSession> startSession(final ClientSessionOptions options) {
        return Mono.from(clientSessionHelper.createClientSession(notNull("options", options), executor))
                .switchIfEmpty(Mono.create(sink ->
                        sink.error(new MongoClientException("Sessions are not supported by the MongoDB cluster to which this client is connected"))));
    }
