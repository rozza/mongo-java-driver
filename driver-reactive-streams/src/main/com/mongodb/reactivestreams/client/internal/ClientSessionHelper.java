...
public class ClientSessionHelper {
    ...
    void withClientSession(@Nullable final AsyncClientSession clientSessionFromOperation, final OperationExecutor executor,
        ...
        if (clientSessionFromOperation != null) {
            ...
            createClientSessionMono(ClientSessionOptions.builder().causallyConsistent(false).build(), executor)
                    .doOnSuccess(clientSession -> {
                        if (clientSession != null) {
                            callback.onResult(clientSession.getWrapped(), null);
                        } else {
                            callback.onResult(null, null);
                        }

                    })
                    .doOnError(e -> callback.onResult(null, e))
                    .subscribe();
        }
    }
    ...
    Mono<ClientSession> createClientSessionMono(final ClientSessionOptions options, final OperationExecutor executor) {
        ...
        if (!getServerDescriptionListToConsiderForSessionSupport(clusterDescription).isEmpty()
            ...
            return Mono.create((MonoSink<Publisher<ClientSession>> sink) -> {
                mongoClient.getCluster().selectServerAsync(this::getServerDescriptionListToConsiderForSessionSupport,
                                                           (server, t) -> {
                                                               if (t != null) {
                                                                   sink.success();
                                                               } else if (server.getDescription()
                                                                       .getLogicalSessionTimeoutMinutes() == null) {
                                                                   sink.success();
                                                               } else {
    ...
    private Mono<ClientSession> createClientSessionPublisher(final ClientSessionOptions options, final OperationExecutor executor) {
