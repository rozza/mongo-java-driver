...
final class AggregatePublisherImpl<T> extends BatchCursorPublisher<T> implements AggregatePublisher<T> {
    ...
    public Publisher<Void> toCollection() {
        ...
        return getMongoOperationPublisher().createReadOperationMono(
                (asyncOperations) -> asyncOperations.getTimeoutContext(maxTimeMS, maxAwaitTimeMS),
    ...
    @Override
    Function<AsyncOperations<?>, TimeoutContext> getTimeoutContext() {
        return (asyncOperations -> asyncOperations.getTimeoutContext(maxTimeMS, maxAwaitTimeMS));
    }
