...
class MongoClientFactoryTest extends FunctionalSpecification {
    ...
    void shouldCreateMongoClientFromEnvironment() throws Exception {
        ...
        MongoClient client = (MongoClient) mongoClientFactory.getObjectInstance(null, null, null, environment);
        try {
    ...
    void shouldCreateMongoClientFromReference() throws Exception {
        Hashtable<String, String> environment = new Hashtable<>();
        Reference reference = new Reference(null, new StringRefAddr("connectionString", Fixture.getMongoClientURIString()));

        MongoClient client = (MongoClient) mongoClientFactory.getObjectInstance(reference, null, null, environment);
        try {
            assertNotNull(client);
        } finally {
            if (client != null) {
