...
private[codecs] object CaseClassCodec {
  ...
  def createCodecBasicCodecRegistryEncodeNone[T: c.WeakTypeTag](c: whitebox.Context)(): c.Expr[Codec[T]] = {
    import c.universe._
    createCodecBasicCodecRegistry[T](c)(c.Expr[Boolean]("true"))
  }
  ...
  def createCodecEncodeNone[T: c.WeakTypeTag](
    ...
    import c.universe._
    createCodec[T](c)(codecRegistry, c.Expr[Boolean]("true"))
  }

  def createCodecBasicCodecRegistryIgnoreNone[T: c.WeakTypeTag](c: whitebox.Context)(): c.Expr[Codec[T]] = {
    import c.universe._
    createCodecBasicCodecRegistry[T](c)(c.Expr[Boolean]("false"))
  }
  ...
  def createCodecIgnoreNone[T: c.WeakTypeTag](
    ...
    import c.universe._
    createCodec[T](c)(codecRegistry, c.Expr[Boolean]("false"))
  }
  ...
  def createCodec[T: c.WeakTypeTag](
            ...
            .map {
              case (p, i) =>
                if (p.isParamWithDefault) {
                  ...
                  p.name -> '{ "${tpe.typeSymbol.companion}.$getterName" }
                } else {
                  c.abort(
                    c.enclosingPosition,
        ...
        case (name, f) =>
          ...
          '{
            typeArgs += ($key -> {
              ...
              ${ flattenTypeArgs(f).map(t => '{ tpeArgs += classOf[${if (isCaseClass(t)) t.finalResultType else t.finalResultType.erasure}] } ) }
              tpeArgs.toList
            })
          }
      })
     ...
     */
    def caseClassesMap: Map[String, Class[_]] = {
      '{
        val caseClassesMap = mutable.Map[String, Class[_]]()
        knownTypes.map(t => caseClassesMap += (${keyName(t)} -> classOf[${t.finalResultType.erasure}]))
    ...
    def classToCaseClassMap = {
      ...
      '{
        ...
        flattenedFieldTypes.map(t =>
          classToCaseClassMap ++= flattenTypeArgs(t)
            .map(t => classOf[${t.finalResultType.erasure}], ${isCaseClass(t) || isCaseObject(t) || isSealed(t)})
        )
    ...
    def writeValue: Tree = {
      val cases: Seq[Tree] = {
        fields.map {
          case (classType, _) if isCaseObject(classType) => '{ ${keyName(classType)} }
      ...
      '{
        ...
        className match
                  case (classType, _) if isCaseObject(classType) => '{ ${keyName(classType)} } =>
          case (classType, fields) => {
           if isCaseObject(classType)

      }
        cq""" ${keyName(classType)} =>
                  val instanceValue = value.asInstanceOf[${classType}]
                  ..${writeClassValues(fields, ignoredFields(classType))}"""
          case _ => throw new BsonInvalidOperationException("Unexpected class type: " + className)
