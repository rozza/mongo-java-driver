...
class MonthCodecSpecification extends Specification {

    def 'should round trip Month successfully'() {
        given:
        def codec = new MonthCodec()
        ...
        then:
        writer.getDocument().get('month').isInt32()

        when:
        BsonReader bsonReader = new BsonDocumentReader(writer.getDocument())
        bsonReader.readStartDocument()
        bsonReader.readName()
        java.time.Month actual = codec.decode(bsonReader, DecoderContext.builder().build())
        ...
        where:
        year << [
            java.time.YearMonth.now().getMonth(),
