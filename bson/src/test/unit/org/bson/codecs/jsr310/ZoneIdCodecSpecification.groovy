...
class ZoneIdCodecSpecification extends Specification {

    def 'should round trip ZoneId successfully'() {
        given:
        def codec = new ZoneIdCodec()
        ...
        then:
        writer.getDocument().get('zoneId').isString()

        when:
        BsonReader bsonReader = new BsonDocumentReader(writer.getDocument())
        bsonReader.readStartDocument()
        bsonReader.readName()
        java.time.ZoneId actual = codec.decode(bsonReader, DecoderContext.builder().build())
        ...
        zoneId << [
            ...
            java.time.ZoneId.of(java.time.ZoneOffset.MIN.getId()),
            java.time.ZoneId.of(java.time.ZoneOffset.MAX.getId())
        ]
    }
